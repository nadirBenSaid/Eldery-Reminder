...
import fr.core.android.ensanotes.models.misc.PlayStoreMetadataFetcherResult;
import fr.core.android.ensanotes.utils.ConnectionManager;
...
public class UpdaterTask extends AsyncTask<String, Void, Void> {
	...
	protected void onPreExecute() {
		...
		if (ensaNotes.isDebugBuild() || !ConnectionManager.internetAvailable(ensaNotes.getAppContext()) || now < prefs.getLong(Constants
				...
				.callback(new MaterialDialog.ButtonCallback() {
					...
					public void onPositive(MaterialDialog materialDialog) {
						if (isGooglePlayAvailable()) {
							((ensaNotes)mActivity.getApplication()).getAnalyticsHelper().trackEvent(AnalyticsHelper.CATEGORIES.UPDATE, "Play Store");
							mActivityReference.get().startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse
									("market://details?id=" + mActivity.getPackageName())));
						} else {
							((ensaNotes)mActivity.getApplication()).getAnalyticsHelper().trackEvent(AnalyticsHelper.CATEGORIES.UPDATE, "Drive Repository");
