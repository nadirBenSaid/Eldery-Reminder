...
package fr.core.android.ensanotes.helpers;
...
import fr.core.android.ensanotes.models.StatsSingleNote;
import fr.core.android.ensanotes.utils.Constants;
import fr.core.android.ensanotes.utils.StorageHelper;
import fr.core.android.ensanotes.utils.TagsHelper;
...
public class NotesHelper {
    ...
    public static void addAttachments(boolean keepMergedNotes, Note note, ArrayList<Attachment> attachments) {
        if (keepMergedNotes) {
            for (Attachment attachment : note.getAttachmentsList()) {
                attachments.add(StorageHelper.createAttachmentFromUri(ensaNotes.getAppContext(), attachment.getUri
                        ()));
            }
        } else {
	...
	public static StatsSingleNote getNoteInfos(Note note) {
		...
		int words, chars;
		if (note.isChecklist()) {
			infos.setChecklistCompletedItemsNumber(StringUtils.countMatches(note.getContent(), fr.core.android.checklistview
					.interfaces.Constants.CHECKED_SYM));
			infos.setChecklistItemsNumber(infos.getChecklistCompletedItemsNumber() +
					StringUtils.countMatches(note.getContent(), fr.core.android.checklistview.interfaces.Constants.UNCHECKED_SYM));
		}
	...
	private static String sanitizeTextForWordsAndCharsCount(Note note, String field) {
		if (note.isChecklist()) {
			String regex = "(" + Pattern.quote(fr.core.android.checklistview.interfaces.Constants.CHECKED_SYM) + "|"
					+ Pattern.quote(fr.core.android.checklistview.interfaces.Constants.UNCHECKED_SYM) + ")";
